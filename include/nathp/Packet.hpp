...
namespace nathp
{
	class Packet
	{
		public:
			...
			template<typename T> void setPayload(T& data);
			template<typename T> T getPayload(void);
			template<typename T> void setPayload(std::vector<T>& payload);
			template<typename T> void getPayload(std::vector<T>& dest);

			template<typename T> int sendVia(T const& connection) const noexcept;
	...
	template<typename T> void Packet::setPayload(T& data)
	{
	...
	template<typename T> T Packet::getPayload(void)
	{
		T result = *reinterpret_cast<T*>(this->payload.data());
		return result;
	}

	template<typename T> void Packet::setPayload(std::vector<T>& payload)
	{
		this->vector_copy(payload, this->payload);
	}

	template<typename T> void Packet::getPayload(std::vector<T>& dest)
	{
		this->vector_copy(this->payload, dest);
	}

	template<typename T, typename U> void Packet::vector_copy(std::vector<T>& src, std::vector<U>& dest)
	{
	...
	template<typename T> int Packet::sendVia(T const& connection) const noexcept
	{
