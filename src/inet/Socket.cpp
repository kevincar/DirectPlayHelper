...
Socket::Socket(int f, int t, int p) : family(f), type(t), protocol(p) {
  ...
  if (this->socket == INVALID_SOCKET) {
    ...
    throw std::out_of_range(
        std::string("Socket::Socket() Error opening socket: ") +
        (std::to_string(err)));
  }
  Socket::n_sockets++;
}

Socket::Socket(int capture, int f, int t, int p)
    : socket(capture), family(f), type(t), protocol(p) {
  if (Socket::n_sockets < 1) this->startup();
  Socket::n_sockets++;
}
...
Socket::operator int() const { return this->socket; }

int Socket::close(void) {
  int result = _CLOSE(this->socket);
  if (result == SOCKET_ERROR) {
    int const err = ERRORCODE;
    std::cout << std::string("Socket::~Socket() Error closing socket: ") +
                     std::to_string(err)
              << std::endl;
    return -1;
  }
...
void Socket::startup(void) {
  ...
  if (err != 0) {
    throw std::logic_error(std::string("Socket::Failed to initialize WSA: ") +
                           std::to_string(err));
  }
#endif /* HAVE_WINSOCK2_H */
  return;
