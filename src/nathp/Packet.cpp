...
unsigned char* Packet::data(void) const {
  ...
  _Packet* _packet;
  _packet = reinterpret_cast<_Packet*>(std::malloc(this->size()));
...
void Packet::setData(unsigned char const* data, unsigned int size) {
  ...
  _Packet const* _packet = reinterpret_cast<_Packet const*>(data);
  ...
  std::copy(_packet->payload, _packet->payload + _packet->payloadSize,
            this->payload.begin());
  return;
}
}  // namespace nathp
