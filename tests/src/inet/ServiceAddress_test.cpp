...
TEST(ServiceAddressTest, constructor) {
  ...
  ASSERT_NO_THROW({ inet::ServiceAddress addr{}; });

  // Good construction
  ASSERT_NO_THROW({ inet::ServiceAddress addr{"192.168.1.100:80"}; });

  // Bad Construction
  ASSERT_THROW({ inet::ServiceAddress addr{"18293.2"}; }, std::out_of_range);
