...
void goldClientProc(nathp::Client& cli) noexcept;
...
std::unique_ptr<std::thread> startTestNATHPServer(int nExpectedClients, std::string& status, std::mutex& status_mutex, std::condition_variable& status_cv)
{
	...
	return std::make_unique<std::thread>([&, nExpectedClients]
	{
		...
		{
			//LOG(DEBUG) << "Current n connected: " << server->getClientList().size();
			SLEEP(250);
		}
		...
		//LOG(DEBUG) << "Server: Clients are connected... " << server->getClientList().size();
		status = "Clients are connected";
		status_lock.unlock();
		status_cv.notify_all();

		//LOG(DEBUG) << "Server: Waiting for client so successfully receive request";
		status_lock.lock();
		status_cv.wait(status_lock, [&]{return status == "Client list request received";});
		status_lock.unlock();
		SLEEP(250);
		{
			std::lock_guard<std::mutex> done_lock {done_mutex};
...
std::unique_ptr<std::thread> startTestNATHPClient(int clientNumber, std::string& status, std::mutex& status_mutex, std::condition_variable& status_cv)
{
	...
	return std::make_unique<std::thread>([&, clientStr, clientNumber]
	{
		...
		//LOG(DEBUG) << clientStr << "Connected! Waiting for server to be ready...";
		std::unique_lock<std::mutex> status_lock {status_mutex};
