...
std::unique_ptr<std::thread> startTestNATHPServer(
    int nExpectedClients, std::string const& status,
    std::mutex const& status_mutex, std::condition_variable const& status_cv) {
...
std::unique_ptr<std::thread> startTestNATHPClient(
    int clientNumber, std::string const& status, std::mutex const& status_mutex,
    std::condition_variable const& status_cv) {
